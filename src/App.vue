  ...
  <Transition name="preloader">
    ...
    >
      ...
      <Transition name="terminal-phase">
        <div v-if="phase === 'terminal'" class="absolute flex flex-col items-center w-full max-w-lg px-4 sm:px-0">
          <!-- Premium loader animation -->
          <div class="relative w-16 h-16 sm:w-24 sm:h-24 mb-8 sm:mb-12 flex items-center justify-center">
            <!-- Outer glow -->
            ...
            <!-- Center element -->
            <div class="w-2 h-2 sm:w-3 sm:h-3 bg-emerald-500 rounded-full animate-ping shadow-[0_0_10px_rgba(16,185,129,0.8)]"></div>
          </div>

          <div class="w-full flex flex-col items-start gap-2 sm:gap-3">
            <template v-for="(line, index) in loadingLines" :key="index">
              <div class="w-full text-left text-emerald-500/90 text-sm sm:text-lg md:text-xl tracking-wider">
            ...
            <!-- Initial Cursor -->
            <div v-if="loadingLines.length === 0" class="text-emerald-500/90 text-sm sm:text-lg md:text-xl tracking-wider">
              <span class="mr-2 sm:mr-3 opacity-60">root@portfolio:~#</span>
              <span class="inline-block w-2 h-4 sm:w-2.5 sm:h-5 bg-emerald-500 align-middle animate-[pulse_0.8s_infinite]"></span>
            </div>
          </div>
        </div>
      </Transition>
      ...
      <Transition name="welcome-phase">
        <div v-if="phase === 'welcome'" class="absolute w-full px-4 flex flex-col items-center justify-center">
          <div class="text-4xl sm:text-6xl md:text-8xl lg:text-9xl text-emerald-500 font-bold tracking-[0.15em] sm:tracking-[0.2em] md:tracking-[0.3em] text-center drop-shadow-[0_0_20px_rgba(16,185,129,0.8)] pl-[0.15em] sm:pl-[0.2em] md:pl-[0.3em] w-full">
            WELCOME
          </div>
          <!-- Hacker-style expanding underscore -->
          <div class="w-3/4 sm:w-full max-w-lg h-1 md:h-2 bg-emerald-500 mt-4 sm:mt-6 md:mt-8 rounded-full shadow-[0_0_15px_rgba(16,185,129,0.8)] animate-[expand-width_0.6s_ease-out_forwards]"></div>
        </div>
      </Transition>
